$(document).ready(function() {
	...
	$('#step1_form').submit(function(e) {
		...
		$.post('/getip', params, function(data) {
			...
			ip = data.ip;
			$('#step2_ip').text(ip);
			var autohostname = 'gogs' + (1000000 + Math.floor(Math.random() * 8999999)) + '.lndyn.com';
	...
	$('#step2_form').submit(function(e) {
		...
		if(hostname.length == 0) {
			...
			showError('Please enter just the hostname (e.g., "example.com" or "gogs.example.com").')
			return;
		} else if(/^[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*$/.test(hostname)) {
			showError('Please enter a hostname like "example.com" or "gogs.example.com", not an IP address.');
			return;
		}
